...
public class ForgeEventHandler {
	...
	public void onServerChat(ServerChatEvent event) {
		String message = OneTenTwoMod.discordChat.filterMCMessage(event.getMessage());
		if (message != null) {
			OneTenTwoMod.discordChat.sendMessage(OneTenTwoMod.discordChat.getFormatter().fromMC(event.getPlayer().getName(), message));
		}
	}

	@SubscribeEvent
	public void onLivingDeath(LivingDeathEvent event) {
		if (OneTenTwoMod.config.sendDeathMessages() && event.getEntityLiving() instanceof EntityPlayer && !(event.getEntityLiving() instanceof FakePlayer) && !event.getEntity().world.isRemote) {
